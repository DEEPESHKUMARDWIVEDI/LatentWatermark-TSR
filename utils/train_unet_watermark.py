...
def train_unet_watermark(train_loader, device,
    ...
    load_checkpoint(ae, None, autoencoder_ckpt, map_location=device)  # loads model only if optimizer None
    ae.eval()
    ...
    for epoch in range(num_epochs):
        ...
        for imgs, _ in tqdm(train_loader, desc=f"Unet Epoch {epoch+1}/{num_epochs}"):
            ...
            with torch.no_grad():
                z, _ = ae(imgs)               # z: [B, latent_dim]

            Iw = unet(imgs, z)
            loss = vgg_loss(imgs, Iw)
        ...
        # Save some watermarked images for later noise-layer training
        unet.eval()
        with torch.no_grad():
            ...
            # save grid (map back to [0,1])
            save_image((sample_Iw + 1.0)/2.0, os.path.join(save_watermarked_dir, f"epoch{epoch+1}_watermarked.png"), nrow=4)
            # optionally also save latent vectors
            torch.save(sample_z.cpu(), os.path.join(save_watermarked_dir, f"epoch{epoch+1}_latents.pt"))

        # checkpoint
        ckpt_path = os.path.join(ckpt_dir, f"unet_epoch{epoch+1}.pth")
        torch.save({"epoch": epoch+1, "model_state": unet.state_dict(), "loss": avg_loss}, ckpt_path)

        if avg_loss < best_loss:
            best_loss = avg_loss
            best_path = os.path.join(ckpt_dir, "unet_best.pth")
            torch.save({"epoch": epoch+1, "model_state": unet.state_dict(), "loss": avg_loss}, best_path)
